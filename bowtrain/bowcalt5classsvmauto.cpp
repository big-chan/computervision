...
void printLabel(Mat trainLabelsMat) {
	for (int i = 0; i < trainLabelsMat.rows;i++) {
		cout << trainLabelsMat.at<float>(i, 0) << "\n" << endl;
	}
}
...
vector<int> SVMevaluate(Mat &testResponse, float &count, float &accuracy, vector<string> &testLabels,string testLabel,int what) {
	...
	accuracy = (count / numzip[what])*100;
...
void bowtrain(int what) {
	...
	for (int i = 0; i < labelzip.size();i++) {
		...
		numzip=SVMevaluate(testResponse, count, accuracy, trainLabels, labelzip[i],i);
		allcount += numzip[i] * (int)accuracy;
	}
	
	cout << "allaccuracy=" << (allcount / trainLabels.size())<< endl;
