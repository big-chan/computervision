...
const string paramsFile = "param.xml";
...
void VocData::getClassImages_impl(const string& obj_class, const string& dataset_str, vector<ObdImage>& images, vector<char>& object_present)
{
	...
	cout <<"filename______"<< gtFilename.find("%s") << endl;
...
void VocData::initVoc2007to2010(const string& vocPath, const bool useTestDataset)
{
	...
	m_sampled_ap = ((m_vocName == "VOC2007") );
}
...
void VocData::readClassifierGroundTruth(const string& filename, vector<string>& image_codes, vector<char>& object_present)
{
	...
	cout <<"1111111111111"<< filename << endl;
	string line;
	string image;
	int obj_present = 0;
	int max1 = 0;
...
struct DDMParams
{
	DDMParams() : detectorType("SIFT"), descriptorType("SIFT"), matcherType("BruteForce") {}
...
{
	VocabTrainParams() : trainObjClass("person"), vocabSize(1000), memoryUse(200), descProportion(0.3f) {}
	...
	const Ptr<FeatureDetector>& fdetector, const Ptr<DescriptorExtractor>& dextractor)
{
	...
	if (!readVocabulary(filename, vocabulary))
	{
		...
		const int bytesInMB =16284*2*16;
	...
	const string& resPath)
{
	...
	for (size_t imageIdx = 0; imageIdx < images.size(); imageIdx++)
	{
		...
		cout << "score___" << confidences[imageIdx] << endl;
	}
...
int main(int argc, char** argv)
{
	...
	const string vocPath = "C:/Users/gkseo/source/repos/Project15/Project15/VOCdevkit/VOC2010", resPath = "C:/Users/gkseo/source/repos/Project15/Project15/res";
	...
	else
	{
		...
		ddmParams = DDMParams("SIFT", "SIFT", "BFMatcher"); // from command line
	...
	Ptr<Feature2D> featureDetector = createByName("SIFT");
	...
	{
		Ptr<BFMatcher> descMatcher=BFMatcher::create();
	...
	for (size_t classIdx = 0; classIdx < objClasses.size(); ++classIdx)
	{
		...
		cout << "size__"<<classIdx <<"____"<< objClasses.size() <<endl;
